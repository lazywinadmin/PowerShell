...
#>
	...
	PARAM (
		[parameter(mandatory)]
		#[validateset('User','Computer', 'Contact', 'Group', 'PublicFolder', 'OrganizationalUnit')]
		[validateset('Group')]
	...
	PROCESS
	{
		...
		IF ($SizeLimit) { $Searcher.SizeLimit = $SizeLimit }
...
Get-ADSIObject -ObjectCategory Group -Identity 'admin'
